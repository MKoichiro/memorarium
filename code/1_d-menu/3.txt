...
// risezeされるたびにオーバーフロー判定をやり直しスクロールの可否を設定 ////////
const delay = 500;
let timeoutId = 0;
window.addEventListener('resize', () => {
  if (timeoutId > 0) { clearTimeout(timeoutId) }
  timeoutId = setTimeout(() => {
    const ul = document.querySelector('ul.menu-lists');
    if (hasMenuOverflow(ul)) {
      ul.addEventListener(  'wheel',        reviveScroll,   {passive: false} );
      ul.addEventListener(  'touchmove',    reviveScroll,   {passive: false} );
    } else {
      ul.removeEventListener(  'wheel',     reviveScroll,   {passive: false} );
      ul.removeEventListener(  'touchmove', reviveScroll,   {passive: false} );
    }
  }, delay);
});
////////////////////////////////////////////////////////////////////////////////